...
import { Laptop, Scissors, Utensils, Wrench, Zap, Clock, Coins, Briefcase, CheckCircle2 } from 'lucide-react';
...
const bgColorsMap: Record<string, string> = {
  'ac-repair': 'bg-sky-50 dark:bg-sky-950/30 border-sky-100 dark:border-sky-900/40',
  'computer': 'bg-indigo-50 dark:bg-indigo-950/30 border-indigo-100 dark:border-indigo-900/40',
  'tailoring': 'bg-pink-50 dark:bg-pink-950/30 border-pink-100 dark:border-pink-900/40',
  'cook': 'bg-amber-50 dark:bg-amber-955/20 border-amber-100 dark:border-amber-900/30',
  'wiring': 'bg-yellow-50 dark:bg-yellow-950/30 border-yellow-101 dark:border-yellow-905/30',
};
...
export default function Programs() {
  ...
  // Handler to set enrolling course and scroll to the form smoothly
  ...
  return (
    <section id="programs" className="py-20 bg-slate-50 dark:bg-slate-900/50 transition-colors duration-350">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 id="programs-heading" className="font-sans text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-4">
            {t.programs.title}
          </h2>
          <div className="w-16 h-1 bg-blue-600 mx-auto rounded mb-5" />
          <p id="programs-lead" className="text-base sm:text-lg text-slate-600 dark:text-slate-350 leading-relaxed">
            {t.programs.subtitle}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {t.courses.map((course: Course, index: number) => {
            const iconObj = iconsMap[course.id] || <Wrench className="h-6 w-6" />;
            const cardBg = bgColorsMap[course.id] || 'bg-blue-50 dark:bg-blue-950/30';
            ...
            return (
              ...
              >
                ...
                <div className={`p-6 flex items-center gap-4 border-b ${cardBg}`}>
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="space-y-2 mb-6">
                      <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-450">
                        ...
                        <span>
                          <strong className="text-slate-700 dark:text-slate-300">{t.programs.durationLabel}:</strong> {course.duration}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-450">
                        <Coins className="h-4 w-4 shrink-0 text-slate-400" />
                        <span>
                          <strong className="text-slate-700 dark:text-slate-300">{t.programs.priceLabel}:</strong> {course.price}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 mb-6">
                      {course.highlights.map((highlight, highlightIdx) => (
                        <div key={highlightIdx} className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-400">
                  ...
                  <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-900">
                    ...
                    <button
                      ...
                      className="w-full py-2.5 px-4 bg-slate-100 hover:bg-blue-600 dark:bg-slate-900 dark:hover:bg-blue-600 text-slate-850 dark:text-slate-200 hover:text-white dark:hover:text-white font-sans font-semibold text-sm rounded-xl transition-all duration-200 flex items-center justify-center gap-1.5 focus:ring-2 focus:ring-blue-500/40"
                    >
